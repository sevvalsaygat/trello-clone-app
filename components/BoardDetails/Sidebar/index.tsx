...
const Sidebar: React.FC<BoardDetailsSidebarPropTypes> = () => {
  ...
  const { t } = useLocale();
  const [travelWorkspacesDetailsIsActive, setTravelWorkspacesDetailsIsActive] =
    useState(true);

  const travelWorkspacesDetailsHandleClick = () => {
    setTravelWorkspacesDetailsIsActive(!travelWorkspacesDetailsIsActive);
  };
    ...
    <React.Fragment>
      {travelWorkspacesDetailsIsActive ? (
        ...
        >
          ...
          <div className="flex w-4 board-details-sidebar h-screen bg-gray-950 group-hover:bg-zinc-950 border-r border-blue-100"></div>
        </div>
      )}
    </React.Fragment>
  );
